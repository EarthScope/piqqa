...
def checkAvailability(
    ...
    if thisAvDF.empty:
        ...
        thisEnd = (
            datetime.datetime.strptime(thisEnd, "%Y-%m-%dT%H:%M:%S")
        ...
        if thisAvDF.empty:
            print(
                f"         WARNING: Unable to retrieve values for requested channels, bypassing"
            )
            thisAvDF.rename(
                columns={"percent_availability": "availability"}, inplace=True
            )

        else:
            print(
                "            Note: Using an average of percent_availability instead of ts_percent_availability_total"
            )

            thisAvDF = (
                thisAvDF.groupby(["snclq", "target", "station"]).mean().reset_index()
            )
            thisAvDF.rename(
                columns={"percent_availability": "availability"}, inplace=True
            )
...
def doAvailability(
    ...
    topStationsDict = (
        {}
    )  # list of stations in top X (default 15) for channel group, or all stations if the channel group is small
    services_used = (
        list()
    )  # used to know which services to link to at the bottom of the availability section

    totalElapsedTime = (
        datetime.datetime.fromisoformat(endDate).timestamp()
        - datetime.datetime.fromisoformat(startDate).timestamp()
    ) / 86400
    ...
    allMetadataDF = reportUtils.getMetadata(
        network, stations, locations, channels, startDate, endDate, "channel"
    )
    ...
    for channelGroup in actualChannels:
        ...
        if doPctAvail:
            ...
            pctAvDF2 = reportUtils.addMetricToDF(
                "percent_availability",
                pctAvDF2,
                network,
                stations,
            ...
            if pctAvDF2.empty:
                print(
                    f"**** WARNING: No Percent Availability found for {channelGroup} - services could be down or metrics may not exist for data yet ****"
                )
                continue

            pctAvDF2 = (
                pctAvDF2.groupby(["snclq", "target", "station"]).mean().reset_index()
            )
            pctAvDF2.rename(
                columns={"percent_availability": "availability"}, inplace=True
            )
            ...
            if len(pctAvDF.target.unique()) < len(tmpMetadataDF.target.unique()):
                print(
                    f"    WARNING: Unable to retrieve percent availability information for {len(tmpMetadataDF.target.unique()) - len(pctAvDF.target.unique())} stations:"
                )
                avStations = pctAvDF.target.unique()
                missingStations = tmpMetadataDF[
                    ~tmpMetadataDF["target"].isin(avStations)
                ].target.values
        ...
        try:
            pctAvDF["slc"] = [
                x + "." + y
                for x, y in zip(
                    [i for i in pctAvDF["station"]],
                    [i.split(".")[-1] for i in pctAvDF["target"]],
                )
            ]
            pctAvDF.sort_values(
                ["availability", "station"], inplace=True, ascending=[False, True]
            )
        ...
        if nsta > nBoxPlotSta:
            ...
            topStationsAvDF, services = reportUtils.getAvailability(
                topStations["snclq"], startDate, endDate, tolerance, ""
            )
            ...
            for station in topStationList:
                ...
                try:
                    thisData = topStationsAvDF[
                        topStationsAvDF["staloc"] == str(station)
                    ]
                    ...
                    for idx, line in thisData.iterrows():
                        datalines.append(
                            [
                                (mpl.dates.date2num(line["earliest"]), stn),
                                (mpl.dates.date2num(line["latest"]), stn),
                            ]
                        )
                        ...
                        if doGaps:
                            if pullThis == "first":
                                ...
                                if gapEnd - gapStart < totalElapsedTime * gapBuffer:
                                    thisGap.append(
                                        (gapStart + (totalElapsedTime * gapBuffer), stn)
                                    )
                                else:
                ...
                if thisLocation == "--":
                    thisLocation = ""
                thisMetadata = tmpMetadataDF[
                    (tmpMetadataDF["station"] == thisStation)
                    & (tmpMetadataDF["location"] == thisLocation)
                ]
                for idx, line in thisMetadata.iterrows():
                    if pd.isnull(line["endtime"]):
                        metadatalines.append(
                            [
                                (mpl.dates.date2num(line["starttime"]), stn),
                                (mpl.dates.date2num(datetime.datetime.now()), stn),
                            ]
                        )
                    else:
                        metadatalines.append(
                            [
                                (mpl.dates.date2num(line["starttime"]), stn),
                                (mpl.dates.date2num(line["endtime"]), stn),
                            ]
                        )

                stn += 1

            DataLines = mc.LineCollection(
                datalines, linewidths=4, color=(0.3, 0.5, 0.7, 1)
            )
            MetadataLines = mc.LineCollection(
                metadatalines, linewidths=10, color=(0.9, 0.9, 0.9, 1)
            )
            ...
            bottomLabels = [
                f"{a} ({b:.3f}%)"
                for a, b in zip(
                    bottomStations["station"], bottomStations["availability"]
                )
            ]
            ...
            for station in bottomStationList:
                ...
                if (
                    not bottomStations[
                        bottomStations["station"] == station
                    ].availability.values
                    == 0
                ):
                    thisData = bottomStationsAvDF[
                        bottomStationsAvDF["staloc"] == str(station)
                    ]
                    if len(thisData.index) > 1:
                        # then there are gaps
                        doGaps = True
            ...
            ax1.set_title(
                f"Availability for {network}, {startDate} to {endDate}\nTop/bottom {nTop} stations"
            )
            ...
            allStationsAvDF, services = reportUtils.getAvailability(
                pctAvDF["snclq"], startDate, endDate, tolerance, ""
            )
            for service in services:
                if service not in services_used:
                    services_used.append(service)
            ...
            for station in allStationList:
                ...
                if not pctAvDF[pctAvDF["station"] == station].availability.values == 0:
                    ...
                    thisData = allStationsAvDF[
                        allStationsAvDF["staloc"] == str(station)
                    ]
                    if len(thisData.index) > 1:
                        # then there are gaps
                        doGaps = True
...
def doBoxPlots(
    ...
    for metric in metricList:
        metricDF = reportUtils.addMetricToDF(
            metric, metricDF, network, stations, locations, channels, startDate, endDate
        )
    ...
    if metricDF.empty:
        print(
            f"        WARNING: No metrics retrieved for {network}.{stations}.{locations}.{channels} {startDate}-{endDate} - services could be down or metrics may not exist for data yet"
        )
        boxPlotDictionary = {}
        actualChannels = list()
        ...
        if "sample_rms" not in metricDF.columns:
            print(
                f"        WARNING: No sample_rms metrics retrieved for {network}.{stations}.{locations}.{channels} {startDate}-{endDate} - metrics may not exist for data yet"
            )
            print(f"        WARNING: Will not be able to select PDFs or Spectrograms")

        # Create a list of all channels that actually have metrics
        actualChannels = sorted(
            list(set([x[3] for x in metricDF["target"].str.split(".")]))
        )
        ...
        if ("ts_num_gaps" in metricDF.columns) or (
            "ts_num_gaps_total" in metricDF.columns
        ):
            print(
                "    INFO: Subsetting ts_num_gaps metric(s) to the data start and end dates for each target"
            )

            ## Get the availability extents for all targets
            extentsDF, service = reportUtils.getAvailability(
                metricDF.snclq.unique(), startDate, endDate, "", "extents"
            )
            ...
            if extentsDF.empty:
                ...
                if "ts_num_gaps" in metricDF.columns:
                    ...
                    for idx, row in extentsDF.iterrows():
                        ...
                        thisStartTime = row["earliest"].replace(
                            hour=0, minute=0, second=0, microsecond=0, nanosecond=0
                        )

                        thisDF = metricDF[
                            (metricDF["target"] == str(thisTarget))
                            & (metricDF["start"] >= thisStartTime.to_datetime64())
                            & (metricDF["start"] <= row["latest"].to_datetime64())
                        ]
                        if tmpDF.empty:
                            tmpDF = thisDF.copy()
                        else:
                    ...
                    columnsToRemove = [
                        c for c in tmpDF.columns if c not in columnsToKeep
                    ]

                    tmpDF.drop(columnsToRemove, axis=1, inplace=True)

                    tmpDF.rename(
                        columns={"ts_num_gaps": "ts_num_gaps_subset"}, inplace=True
                    )
                    ...
                    try:
                        metricDF = pd.merge(
                            ...
                            tmpDF,
                            how="outer",
                            on=["target", "snclq", "station", "start", "end"],
                        )
                        metricDF.drop("ts_num_gaps", axis=1, inplace=True)
                        metricDF.rename(
                            columns={"ts_num_gaps_subset": "ts_num_gaps"}, inplace=True
                        )
                    except:
                        pass
                ...
                if "ts_num_gaps_total" in metricDF.columns:
                    ...
                    extentsDF["target"] = (
                        extentsDF["network"]
                        .str.cat(
                            extentsDF["station"].str.cat(
                                extentsDF["location"].str.cat(
                                    extentsDF["channel"], sep="."
                                ),
                                sep=".",
                            ),
                            sep=".",
                    ...
                    tmpDF.rename(
                        columns={"ts_num_gaps_total": "ts_num_gaps_total_subset"},
                        inplace=True,
                    )
                    metricDF = pd.merge(
                        metricDF, tmpDF, how="outer", on=["target", "start", "end"]
                    )
                    metricDF.drop("ts_num_gaps_total", axis=1, inplace=True)
                    metricDF.rename(
                        columns={"ts_num_gaps_total_subset": "ts_num_gaps_total"},
                        inplace=True,
                    )
        ...
        if "sample_rms" in scaledDF.columns:
            ...
            try:
                ...
                for ind, row in metadataDF.iterrows():
                    ...
                    affectedRows = scaledDF.index[
                        (
                            (scaledDF["target"] == thisTarget)
                            & (scaledDF["start"] < thisEnd)
                            & (scaledDF["end"] > thisStart)
                        )
                    ].tolist()
                    scaledDF.loc[affectedRows, "scale_corrected_sample_rms"] = (
                        scaledDF["sample_rms_orig"][affectedRows] / thisScale
                    )
            except:
                quit(
            ...
            for metric in metricList:
                ...
                try:
                    ...
                    if df2.isnull().values.all():
                        print(
                            f"        WARNING: All stations are null for {channelGroup} {metric}, bypassing"
                        )
                        continue
                    ...
                    if nsta > nBoxPlotSta:
                        ...
                        ax1.set_title(
                            f"{metric}\nTop/bottom {nTop} stations, {plotOrder}"
                        )
...
def doPDFs(
    ...
    for channelGroup in actualChannels:
        ...
        try:
            df2 = pd.DataFrame(
                {col: vals["scale_corrected_sample_rms"] for col, vals in grouped}
            )
        except:
            continue
        ...
        if df2.isnull().values.all():
            print(
                f"    WARNING: All stations are null for {channelGroup} sample_rms, bypassing"
            )
            continue
        meds = df2.median(skipna=True).dropna(how="all")
        meds.sort_values(ascending=True, inplace=True)
        ...
        for ii in range(len(meds.index)):
            ...
            lowestNSLC = lowestTarget[
                :-3
            ]  # strip the 3rd component of the channel code, to pull up the other components of that channel later
        ...
        print(
            f"        --> lowest scaled sample_rms station: {lowestTarget.split('.')[1]}"
        )

        # Get a list of channels that should have noise profiles for this station
        expectedTargets_lowest = reportUtils.retrieveExpectedPDFs(
            lowestNSLC, startDate, endDate
        )
        ...
        for target in expectedTargets_lowest:
            ...
            pdfFile = reportUtils.getPDF(
                target, startDate, endDate, spectPowerRange, imageDir
            )
        ...
        if not lowestNSLC == largestNSL:
            # Get a list of channels that should have noise profiles for this station
            expectedTargets_largest = reportUtils.retrieveExpectedPDFs(
                largestNSL, startDate, endDate
            )

            print(
                f"        --> highest scaled sample_rms station: {largestTarget.split('.')[1]}"
            )
            pdfFiles = list()
        ...
        expectedTargets_all = reportUtils.retrieveExpectedPDFs(
            allTarget, startDate, endDate
        )
        ...
        for channel in expectedChannels:
            file = reportUtils.getPDF(
                f"{network}.{stations}.{locations}.{channel}.?",
                startDate,
                endDate,
                spectPowerRange,
                imageDir,
...
def doSpectrograms(
    ...
    for channelGroup in actualChannels:
        ...
        try:
            ...
            print(
                f"    WARNING: Bypassing {channelGroup} because there was no availability for it for this time period"
            )
            continue

        # Pare down to the channel group and the stations with the greatest availability
        tmpDF = scaledDF[
            scaledDF["target"].str.endswith(channelGroup)
            & scaledDF["station"].isin(topStations)
        ]
        ...
        if df2.isnull().values.all():
            print(
                f"    WARNING: All stations are null for {channelGroup} scaled sample_rms, bypassing"
            )
            continue
        ...
        for target in expectedTargets_lowest:
            ...
            if not userDefinedPowerRange:
                ...
                spectPowerRange = userDefinedPowerRange
            spectFile = reportUtils.getSpectrogram(
                target, startDate, endDate, spectPowerRange, spectColorPalette, imageDir
            )
...
def doMap(
    ...
    try:
        ...
        print("    INFO: Retrieving metadata")
        metadataDF = reportUtils.getMetadata(
            network, stations, locations, channels, startDate, endDate, "station"
        )
        ...
        bounds = [
            metadataDF["Latitude"].max() + latScale,
            metadataDF["Longitude"].min() - lonScale,
            metadataDF["Latitude"].min() - latScale,
            metadataDF["Longitude"].max() + lonScale,
        ]
        ...
        fig = px.scatter_map(
            ...
            color_discrete_sequence=markerColor,
            zoom=zoom,
            height=500,
            size="size",
            size_max=7,
...
def doReport(
    ...
    # Get network description for report header
    netDF = reportUtils.getMetadata(
        network, stations, locations, channels, startDate, endDate, "network"
    )
    netDesc = netDF["Description"][0]
    ...
    introText = (
        ...
        f"and time period specified above. EarthScope' goal in generating these reports is to give PIs for temporary "
    ...
    if splitPlots == 1:
        availabilityIntroText = (
            f"{availabilityIntroText} <br/>The {nTop} stations with the highest percent of data available and {nBottom} stations "
            f"with the lowest percent of data available are plotted. If a channel group has fewer stations that this, then all stations "
            f"will be plotted.<br/>"
        )
    ...
    boxPlotIntroText = (
        f"<p>The boxplots in this section illustrate the statistical distribution of selected MUSTANG metrics, "
    ...
    if "sample_rms" in metricList:
        boxPlotIntroText = (
            f"{boxPlotIntroText}<p>The sample_rms metric is the root-mean-square (RMS) variance (also known as the standard deviation) of sample "
    ...
    if "ts_channel_continuity" in metricList and "num_gaps" in metricList:
        boxPlotIntroText = (
            f"{boxPlotIntroText}<p>The ts_channel_continuity and num_gaps metrics are both measurements of data availability. "
    ...
    if "ts_num_gaps" in metricList:
        boxPlotIntroText = (
            f"{boxPlotIntroText}<p>The ts_num_gaps metric returns the number of gaps per day, with one "
    ...
    if "ts_num_gaps_total" in metricList:
        boxPlotIntroText = (
            f"{boxPlotIntroText}<p>The ts_num_gaps_total returns the sum total number of gaps across the "
    ...
    boxPlotIntroText = (
        f"{boxPlotIntroText}<p>Metric boxplots are generated using the Z component for each channel group (for example, BHZ, HHZ, HNZ) "
        f"for each metric included in the report, and are sorted by their median value. "
        f"This is true for all metrics except for ts_num_gaps and num_gaps (if used), which are sorted by mean value.</p>"
    )

    boxPlotIntroText2 = (
        f"The boxplots contain the following features:<br/>"
        f"<ul>"
    ...
    if splitPlots == 1:
        boxPlotIntroText3 = (
            f"<br/>Within each boxplot, we display the {nTop} highest and the {nBottom} lowest median metric "
            f"values (mean values for the gap metrics listed above) for stations in the network.  "
            ...
            f"containing the {nBottom} lowest median metric values. This is true unless the channel group has fewer than {2*nTop} stations with data, "
            f"in which case all stations are plotted for that group."
        )
    else:
    ...
    if availabilityType == "ts_percent_availability_total":
        spectAvailDescription = (
            "which is agnostic to the metadata extents for the channel"
        )
    else:
        spectAvailDescription = (
            "which are bound by the metadata extents for the channel"
        )
    ...
    if availabilityType == "ts_percent_availability_total":
        spectrgramPlotIntoText = (
            f"{spectrgramPlotIntoText}<li>As a measure of data continuity, omit stations that have less than 75% data availability calculated between the first "
    ...
    spectrgramPlotIntoText = (
        f"{spectrgramPlotIntoText}({availabilityType} metric), unless no station meets this criterion.</li>"
    ...
    mapIntroText = (
        f"Below is a plot of station locations for the network and time period specified at the beginning of this report. "
    ...
    with open(outfile, "w") as f:
        ...
        f.write("    <head>\n")
        f.write(
            f"<meta name=Title content=\"{network} ({' - '. join([startDate, endDate])}) PIQQA Data Quality Report\">\n"
        )
        f.write(
            f"<title>{network} ({' - '. join([startDate, endDate])}) PIQQA Data Quality Report</title>\n"
        )
        f.write("    </head>\n\n")
        ...
        if startYear == endYear:
            f.write(
                f"<a name='top'></a> <h1>PIQQA Data Quality Report for {network} ({startYear})</h1>"
            )
        else:
            f.write(
                f"<a name='top'></a> <h1>PIQQA Data Quality Report for {network} ({startYear}-{endYear})</h1>"
            )

        f.write(f"<h2>{netDesc}</h2>")
        ...
        f.write(
            f'    <center><img src="{boxPlotExampleImage_moved}" width="50%"></center><br/>'
        )
        ...
        for channel in actualChannels:
            ...
            allFiles = (
                pdfDictionary[f"{channel}_smallest"]
                + pdfDictionary[f"{channel}_greatest"]
                + pdfDictionary[f"{channel}_all"]
            )
            if allFiles == []:
                print(
                    f"    WARNING: No PDF plots available for {channel[0:2]}, omitting it from the report"
                )
                continue
            ...
            f.write(
                f"<h3>{channel[0:2]} channels - <a href='{pdfLink}' target='_blank' >PDF Browser</a></h3>"
            )
            f.write('<div class="row">')
            f.write('  <div class="column">')
            if nStations == 2:
        ...
        for channel in actualChannels:
            ...
            # Some channels, such as VM, do not have noise profiles calculated in MUSTANG, and can be bypassed in this section
            allFiles = (
                spectDictionary[f"{channel}_smallest"]
                + spectDictionary[f"{channel}_greatest"]
            )
            if allFiles == []:
                print(
                    f"    WARNING: No spectrogram plots available for {channel[0:2]}, omitting it from the report"
                )
                continue
            ...
            if not userDefinedPowerRange:
                try:
                    spectPowerRange = ",".join(
                        [str(int) for int in powerRanges[channel[0:2]]]
                    )
                except:
                    spectPowerRange = "-200,-20"
            else:
            ...
            f.write(
                f"<h3>{channel[0:2]} channels - <a href='{spectLink}' target='_blank' >Spectrogram Browser</a></h3>"
            )
            if splitPlots == 1:
                f.write(
                    f"<b><center>Selected from the {nTop} stations with the highest availability</center></b><br/>"
                )
        ...
        if mapformat == "html":
            ...
            f.write(
                f'    <center><embed src="{mapFilename}.{mapformat}" width="800px" height="600px" /></center><br/>'
            )
        else:
        ...
        f.write(
            "<tr><td><b>Network</b></td><td><b>Station</b></td><td><b>Latitude</b></td><td><b>Longitude</b></td><td><b>Elevation</b></td><td><b>Site Name</b></td><td><b>Start Date</b></td><td><b>End Date</b></td></tr>"
        )
        ...
        for ind, row in metadataDF.iterrows():
            ...
            f.write(
                f'<tr><td><a href="{mdaLink}" target="_blank" >{row["#Network"]}</a></td>'
            )
            f.write(
                f'<td><a href="{mdaLink}" target="_blank" >{row["Station"]}</a></td>'
            )
            f.write(
                f'<td><a href="{mdaLink}" target="_blank" >{row["Latitude"]:.4f}</a></td>'
            )
            f.write(
                f'<td><a href="{mdaLink}" target="_blank" >{row["Longitude"]:.4f}</a></td>'
            )
            f.write(
                f'<td><a href="{mdaLink}" target="_blank" >{row["Elevation"]:.1f}</a></td>'
            )
            f.write(
                f'<td><a href="{mdaLink}" target="_blank" >{row["SiteName"]}</a></td>'
            )
            f.write(
                f'<td><a href="{mdaLink}" target="_blank" >{row["StartTime"]}</a></td>'
            )
            f.write(
                f'<td><a href="{mdaLink}" target="_blank" >{row["EndTime"]}</a></td></tr>'
            )
...
def doZip(outdir, outfile):
    print(
        f"INFO: Report written to {outdir}/{outfile} at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
...
def main():
    ...
    metricsRequired = (
        []
    )  # requiring percent_availability is obsolete, but leaving this as an option in case something changes in teh future.
    ...
    for arg in sys.argv:
        if arg.lower().startswith("--network="):
            network = arg.split("=")[1]
        elif arg.lower().startswith("--stations=") or arg.lower().startswith(
            "--station="
        ):
            stations = arg.split("=")[1]
        elif arg.lower().startswith("--locations=") or arg.lower().startswith(
            "--location="
        ):
            locations = arg.split("=")[1]
        elif arg.lower().startswith("--channels=") or arg.lower().startswith(
            "--channel="
        ):
        ...
        elif arg.lower().startswith("--metrics=") or arg.lower().startswith(
            "--metric="
        ):
    ...
    try:
        ...
        ireturn = 1
        print(
            "WARNING: Could not parse start date, is it formatted correctly? YYYY-mm-dd"
        )
    try:
        endYear = (
            datetime.datetime.strptime(endDate, "%Y-%m-%d") - datetime.timedelta(days=1)
        ).strftime("%Y")
    except:
        ireturn = 1
        print(
            "WARNING: Could not parse end date, is it formatted correctly? YYYY-mm-dd"
        )
    if ireturn == 1:
        quit("INFO: Exiting PIQQA")

    print(
        "INFO: Beginning Report at "
    ...
    # Create BoxPlots
    [actualChannels, scaledDF, splitPlots, boxPlotDictionary, metricsWithPlots] = (
        doBoxPlots(
            splitPlots,
            metricList,
            metricsRequired,
    ...
    # Create PDF Plots
    [splitPlots, pdfDictionary] = doPDFs(
        splitPlots,
        actualChannels,
        scaledDF,
        startDate,
    ...
    [splitPlots, spectDictionary] = doSpectrograms(
        splitPlots,
        actualChannels,
        topStationsDict,
        scaledDF,
